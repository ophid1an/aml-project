...
from costcla.models import CostSensitiveDecisionTreeClassifier,BayesMinimumRiskClassifier,ThresholdingOptimization,CostSensitiveRandomPatchesClassifier,CostSensitiveRandomForestClassifier
...
X_u, y_u, cost_mat_u = undersampling(X_train, y_train, cost_mat_train) #without cost
#Calibrated models
...
for model in classifiers.keys():
    ...
    #Sampling
    #classifiers[model]["c_o"] =cross_val_predict( classifiers[model]["f"], X_cps_o, y_cps_o)
    ...
    print(brier_score_loss(y_test, classifiers[model]["c_o_p"][:, 1]))
    a = binary_classification_metrics(y_test, classifiers[model]["c_o"], classifiers[model]["c_o_p"][:, 1])
    print(a)
...
for model in classifiers.keys():
    ...
    #results["brier_loss"].loc[model] = brier_score_loss(y_test, classifiers[model]["p"])
    ...
    #results["brier_loss"].loc[model + "-O"] = brier_score_loss(y_test, classifiers[model]["p"])
    ...
    #results["brier_loss"].loc[model + "-R"] = brier_score_loss(y_test, classifiers[model]["p"])
    ...
    #results["brier_loss"].loc[model + "-U"] = brier_score_loss(y_test, classifiers[model]["p"])
...
CostSensitiveClassifiers = {"CSRP": {"f": CostSensitiveRandomPatchesClassifier(combination='weighted_voting')},
                            "RFC": {"f": CostSensitiveRandomForestClassifier()}}
label=["CSRP","RFC"]
...
for model in CostSensitiveClassifiers .keys():
    ...
    print("Model:%20s  savings:%0.6f cost: %0.1f" % (label,savings_score(y_test, CostSensitiveClassifiers[model]["f"], cost_mat_test),cost_loss(y_test, CostSensitiveClassifiers[model]["f"], cost_mat_test)))
...
figsize(10, 5)
ax = plt.subplot(111)
l = ax.plot(ind, results["cost_loss"], "-o", label='F1Score', color=colors[2])
b = ax.bar(ind-0.3, results['cost_loss'], 0.6, label='Savings', color=colors[0])
